...
class ThreadsCog(commands.Cog):
    ...
    async def auto_threads_listener(self, msg: discord.Message):
        ...
        config = await asyncio.to_thread(self._get_channel_config_cached, msg.channel.id)
        if not config:
            return
        ...
        has_media = any(
            ...
            or attachment.filename.lower().endswith(MEDIA_EXTENSIONS)
        ...
        if (
            config_mode == "all"
            and (has_text or has_media)
            or config_mode == "media"
            and has_media
            or config_mode == "text"
            and has_text
            and not has_media
        ...
        if should_create_thread:
            ...
            try:
                ...
                if "thread has already been created" in str(exc).lower():
                    return
