...
class InfoCog(commands.Cog):
    ...
    # ──────────────────────────────────────────────────────────────────────
    @staticmethod
    def _format_badges(public_flags: discord.PublicUserFlags) -> str | None:
        if not public_flags:
            return None
        out = []
    ...
    async def _resolve_target(
    ...
    ) -> tuple[discord.User, discord.Member | None]:
        """
        ...
        if isinstance(target, int):
            try:
                ...
                raise commands.BadArgument("No encontré a ese usuario.") from None
            if ctx.guild:
    ...
    @staticmethod
    async def _try_fetch_member(guild: discord.Guild, user_id: int) -> discord.Member | None:
        try:
    ...
    async def user_info(
        self, ctx: commands.Context, *, member: discord.Member | discord.User | int | None = None
    ):
        """
        ...
        embed = discord.Embed(
            color=base_color or discord.Color.blurple(), timestamp=datetime.datetime.utcnow()
        )
        ...
        embed.set_author(name=f"{mention_or_name} ({display_name})", icon_url=user_profile.display_avatar.url)
    ...
    async def avatar(
        self, ctx: commands.Context, *, target: discord.Member | discord.User | int | None = None
    ):
