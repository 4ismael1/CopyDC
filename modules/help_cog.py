...
def get_help_embeds(lang: str = "es") -> list[discord.Embed]:
    ...
    if lang == "es":
        ...
        embed1.add_field(
            name="`c!get`", value="Extrae la imagen de un emoji o sticker de un mensaje.", inline=False
        )
        ...
        embed2.add_field(
            name="Formatos disponibles", value="Puedes usar `c!thread`, `!thread` o `/thread`.", inline=False
        )
        embed2.add_field(
        ...
        embed2.add_field(
            name="`thread remove #canal`",
            value="Desactiva los hilos automaticos para un canal.",
            inline=False,
        )
        embed2.add_field(
            name="`thread list`", value="Muestra los canales configurados en el servidor.", inline=False
        )
        ...
        embed3.add_field(
            name="Servidor", value="`c!serverinfo` o `c!server` - Estadisticas del servidor.", inline=False
        )
        embed3.add_field(
            name="Avatar", value="`c!avatar` o `c!pfp` - Avatar en grande y descarga.", inline=False
        )
        embed3.add_field(
            name="Roles", value="`c!roleinfo` o `c!role` - Detalle de roles y permisos.", inline=False
        )
        embed3.add_field(
            name="Boost", value="`c!boost` o `c!1boost` - Estado de boost del usuario.", inline=False
        )
        ...
        embed4.add_field(
            ...
            value="Puedes usar `c!counting`, `!counting` o `/counting`.",
            inline=False,
        )
        embed4.add_field(name="`counting set <#canal>`", value="Establece el canal de conteo.", inline=False)
        embed4.add_field(
            name="`counting reset`", value="Reinicia el contador a 0 en el canal actual.", inline=False
        )
        embed4.add_field(
            name="`counting status`", value="Muestra el progreso actual del canal.", inline=False
        )
        embed4.add_field(
            name="Reglas",
            value=(
        ...
        embed5.add_field(
            name="Formatos disponibles", value="Puedes usar `c!react`, `!react` o `/react`.", inline=False
        )
        embed5.add_field(
        ...
        embed5.add_field(
            name="`react clear`", value="Elimina todas las configuraciones con confirmacion.", inline=False
        )
        embed5.add_field(
            name="Comportamiento",
        ...
        embed6.add_field(
            name="Disponible por slash", value="Este modulo se configura con `/boostrole`.", inline=False
        )
        embed6.add_field(
            name="`/boostrole add`", value="Asigna o actualiza un rol para un miembro.", inline=False
        )
        embed6.add_field(name="`/boostrole remove`", value="Quita el rol y borra su registro.", inline=False)
        embed6.add_field(
            name="`/boostrole setlog`", value="Define el canal de logs del modulo.", inline=False
        )
        ...
        embed9 = discord.Embed(
            title="Ayuda: Casos de soporte",
            description="Canales privados de soporte con expediente completo al cierre.",
            color=0x5865F2,
        )
        embed9.add_field(
            name="Configuracion",
            value="`/case setup` define categoria, rol de soporte, archivo privado y retencion.",
            inline=False,
        )
        embed9.add_field(
            name="Uso",
            value="`/case open`, `/case close`, `/case list`, `/case privacy`.",
            inline=False,
        )
        embed9.set_footer(text="Seccion: Casos de soporte")

        embed10 = discord.Embed(
            title="Ayuda: LFG en vivo",
            description="Matchmaking voluntario basado en actividades Playing de Discord.",
            color=0x5865F2,
        )
        embed10.add_field(
            name="Configuracion",
            value="`/lfg setup` publica el panel y `/lfg game_add` vincula actividad y rol.",
            inline=False,
        )
        embed10.add_field(
            name="Participacion",
            value="`/lfg enroll`, `/lfg leave`, `/lfg status`, `/lfg privacy`.",
            inline=False,
        )
        embed10.set_footer(text="Seccion: LFG en vivo")
    ...
    embed2.add_field(
        name="Available formats", value="You can use `c!thread`, `!thread`, or `/thread`.", inline=False
    )
    embed2.add_field(
        name="`thread add #channel <mode>`",
        value="Enable automatic threads. Modes: `all`, `media`, `text`.",
        inline=False,
    )
    embed2.add_field(
        name="`thread remove #channel`", value="Disable automatic threads for a channel.", inline=False
    )
    ...
    embed3.add_field(
        name="Users",
        value="`c!user`, `c!userinfo`, `c!ui` - User information in DMs or server.",
        inline=False,
    )
    embed3.add_field(name="Server", value="`c!serverinfo` or `c!server` - Server statistics.", inline=False)
    embed3.add_field(
        name="Avatar", value="`c!avatar` or `c!pfp` - Large avatar preview and download.", inline=False
    )
    embed3.add_field(
        name="Roles", value="`c!roleinfo` or `c!role` - Role details and permissions.", inline=False
    )
    ...
    embed4.add_field(
        name="Available formats", value="You can use `c!counting`, `!counting`, or `/counting`.", inline=False
    )
    embed4.add_field(name="`counting set <#channel>`", value="Set the counting channel.", inline=False)
    embed4.add_field(
        name="`counting reset`", value="Reset the counter to 0 in the current channel.", inline=False
    )
    embed4.add_field(
        name="`counting status`", value="Show the current progress for this channel.", inline=False
    )
    embed4.add_field(
        name="Rules",
        value=(
    ...
    embed5.add_field(
        name="Available formats", value="You can use `c!react`, `!react`, or `/react`.", inline=False
    )
    embed5.add_field(
        name="`react add <word> <emoji...>`",
        value="Configure up to 20 reactions for a word or phrase.",
        inline=False,
    )
    ...
    embed5.add_field(
        name="Behavior", value="Supports unicode and custom server emojis. Detects whole words.", inline=False
    )
    ...
    embed8.add_field(
        name="Note",
        value="Your server must have an official Discord clan tag for this module to work.",
        inline=False,
    )
    embed8.set_footer(text="Section: Clan Tag")

    embed9 = discord.Embed(
        title="Help: Support cases",
        description="Private support channels with a complete record generated at close.",
        color=0x5865F2,
    )
    embed9.add_field(
        name="Setup",
        value="`/case setup` sets the category, support role, private archive and retention.",
        inline=False,
    )
    embed9.add_field(
        name="Usage",
    ...
    embed9.set_footer(text="Section: Support cases")

    embed10 = discord.Embed(
        title="Help: Live LFG",
        description="Opt-in matchmaking based on Discord Playing activities.",
        color=0x5865F2,
    )
    embed10.add_field(
        name="Setup",
        value="`/lfg setup` publishes the panel and `/lfg game_add` links an activity and role.",
        inline=False,
    )
    embed10.add_field(
        name="Participation",
    ...
    embed10.set_footer(text="Section: Live LFG")
...
def get_select_options(lang: str, current_index: int = 0) -> tuple[list[discord.SelectOption], str]:
    ...
    if lang == "en":
        labels = [
            ...
            ("Support Cases", "Private support transcripts"),
            ("Live LFG", "Real-time game matchmaking"),
        ]
        ...
        labels = [
            ...
            ("Casos de soporte", "Expedientes privados de soporte"),
            ("LFG en vivo", "Grupos por actividad en tiempo real"),
        ]
...
class HelpView(discord.ui.View):
    def __init__(self, *, lang: str = "es", current_index: int = 0):
        ...
        self.add_item(
            discord.ui.Button(label="Privacy", style=discord.ButtonStyle.link, url=PRIVACY_URL, row=2)
        )
