...
intents.presences = True  # detectar cambios en estado personalizado


def total_users_all_guilds(bot: commands.Bot) -> int:
    """Suma de miembros por servidor (puede contar usuarios repetidos entre servidores)."""
...
async def on_ready():
    ...
    log.info(f"👥 Usuarios totales (suma de servidores): {total}")
...
async def on_guild_join(guild):
    ...
    db.add_guild(guild)
    total = total_users_all_guilds(bot)
    log.info(f"➕ Se unió a {guild.name} ({guild.id}) | Total: {len(bot.guilds)} servidores, {total} usuarios")

@bot.event
async def on_guild_remove(guild):
    """Elimina el servidor de la base de datos."""
    # Ignorar si el guild no tiene nombre (evento fantasma al iniciar)
    if not guild.name:
        return
    db.remove_guild(guild)
    total = total_users_all_guilds(bot)
    log.info(f"➖ Salió de {guild.name} ({guild.id}) | Total: {len(bot.guilds)} servidores, {total} usuarios")
