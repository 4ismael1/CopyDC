...
class PermissionsInspectorCog(commands.Cog):
    ...
    async def modcheck(
        ...
        emb.add_field(
            name="¿Vista de moderación aquí?", value="**Sí**" if can_open else "**No**", inline=False
        )
    ...
    @commands.guild_only()
    async def modcheckall(self, ctx: commands.Context, member: discord.Member | None = None):
        ...
        emb.add_field(
            ...
            value=("**Sí**" if can_open_somewhere else "**No**")
            + f" — ({len(modview_channels)}/{total_visible} canales visibles)",
            inline=False,
        )
    ...
    @commands.guild_only()
    async def why_modview(self, ctx: commands.Context, member: discord.Member | None = None):
        ...
        emb.set_thumbnail(url=member.display_avatar.url)
        emb.set_footer(
            text="Incluye permisos de gestión como Gestionar Apodos, Roles, Canales, Mensajes, etc."
        )
        await ctx.reply(member.mention, embed=emb, mention_author=False)
    ...
    @commands.guild_only()
    async def whocan(
        self, ctx: commands.Context, perm_key: str, channel: discord.abc.GuildChannel | None = None
    ):
        ...
        if perm_key not in valid:
            return await ctx.reply(
                f"Permiso no válido. Usa uno de: `{', '.join(valid)}`.", mention_author=False
            )
        if channel is None and isinstance(
            ctx.channel,
            (
