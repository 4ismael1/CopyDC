...
def get_owner_help_embeds() -> list[discord.Embed]:
    ...
    owner_core.add_field(
        name="`c!slashsync [global|guild|all|cleanup]` / `!slashsync ...`",
        value=(
            "Sincroniza slash commands globalmente, en el guild actual o en ambos alcances. "
            "`cleanup` elimina copias antiguas por servidor sin tocar los comandos globales."
        ),
        inline=False,
    )
    owner_core.add_field(
    ...
    devtools.add_field(
        name="`dev reload <mod>`", value="Recarga el modulo o lo carga si no estaba activo.", inline=False
    )
    devtools.add_field(name="`dev reloadall`", value="Recarga todas las extensiones cargadas.", inline=False)
    devtools.add_field(
        name="`dev list [loaded|unloaded|all]`", value="Lista modulos por estado.", inline=False
    )
    devtools.add_field(
        name="`dev info <mod>`", value="Muestra el cog cargado y sus comandos registrados.", inline=False
    )
    ...
    db_health.add_field(
        name="`c!dbhealth` / `!dbhealth`", value="Resumen ejecutivo del estado de DB y trafico.", inline=False
    )
    db_health.add_field(
        name="`dbhealth top`", value="Top de operaciones de DB por frecuencia y latencia.", inline=False
    )
    db_health.add_field(
        name="`dbhealth guilds`", value="Top de servidores por trafico medido por el monitor.", inline=False
    )
    db_health.add_field(
        name="`dbhealth raw`", value="Reporte JSON completo del snapshot y la recomendacion.", inline=False
    )
    db_health.add_field(
        name="`dbhealth reset`", value="Resetea metricas acumuladas del monitor.", inline=False
    )
    ...
    perms.add_field(
        name="`c!modcheck [@usuario] [#canal]`",
        value="Permisos efectivos en el canal actual o indicado.",
        inline=False,
    )
    perms.add_field(
        name="`c!modcheckall [@usuario]`",
        value="Auditoria completa del servidor para ese usuario.",
        inline=False,
    )
    perms.add_field(
        name="`c!why_modview [@usuario]`",
        value="Explica por que ese usuario ve la UI de moderacion.",
        inline=False,
    )
    perms.add_field(
        name="`c!roleperms @rol`", value="Lista permisos de gestion/moderacion del rol.", inline=False
    )
    perms.add_field(
        name="`c!auditroles`",
        value="Audita roles con permisos de gestion y cuantos miembros los tienen.",
        inline=False,
    )
    perms.add_field(
...
class OwnerHelpSelect(ui.Select):
    def __init__(self, embeds: list[discord.Embed]):
        ...
        super().__init__(
            placeholder="Elige una seccion owner...", min_values=1, max_values=1, options=options
        )
        self.embeds = embeds
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(
                "Solo el owner puede usar este menú.",
                ephemeral=True,
            )
            return False
        return True
...
class OwnerHelpCog(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, err: commands.CommandError):
        if (
            isinstance(err, commands.CheckFailure)
            and ctx.command
            and ctx.command.qualified_name.startswith("ohelp")
        ):
            return
    ...
    async def owner_help(self, ctx: commands.Context):
        ...
        view = OwnerHelpView(embeds, ctx.author.id)
