...
def resolve_guild(bot: commands.Bot, query: str) -> discord.Guild | None:
...
def normalize_presence_activity_type(raw: str) -> str | None:
...
def normalize_presence_status(raw: str) -> str | None:
    value = (raw or "").strip().lower()
    aliases = {
        "online": "online",
        "idle": "idle",
...
def build_presence_embed(row: dict | None, *, title: str = "Estado del bot") -> discord.Embed:
...
def build_overview_embed(g: discord.Guild) -> discord.Embed:
    ...
    e.add_field(
        name="Miembros",
        value=f"Total: {total if total is not None else 'N/D'}\n🙂 Humanos: {humans}\n🤖 Bots: {bots}",
        inline=False,
    )
...
def build_roles_emojis_embed(g: discord.Guild) -> discord.Embed:
    ...
    e.add_field(
        name="Icono animado",
        value=fmt_bool(bool(getattr(g, "icon", None) and g.icon.is_animated())),
        inline=True,
    )
...
def build_security_boosts_embed(g: discord.Guild) -> discord.Embed:
    ...
    e.add_field(
        name="Seguridad",
        value=f"Verificación: {v_level}\nNSFW: {nsfw_level}\nFiltro explícito: {explicit_filter}",
        inline=True,
    )
...
def build_bot_perms_embed(g: discord.Guild) -> discord.Embed:
    ...
    checks = {
        ...
        "timeout_members/moderate_members": getattr(p, "moderate_members", False)
        or getattr(p, "timeout_members", False),
    ...
    e.add_field(
        name="Top role del bot",
        value=f"{top_role.name} (pos. {top_role.position})" if top_role else "—",
        inline=False,
    )
    return e
...
class GuildInfoView(ui.View):
    def __init__(
        ...
        guild_id: int,
        owner_id: int,
        back_to_list: tuple[int, int] | None = None,
        timeout: int = 180,
    ):
        """
    ...
    async def btn_back(self, interaction: discord.Interaction, button: ui.Button):
        if not self.back_to_list:
            await interaction.response.edit_message(
                content="No hay lista previa.", embed=self.current_embed(), view=self
            )
            return
...
class ServerListView(ui.View):
    def __init__(
        self, bot: commands.Bot, owner_id: int, page_index: int = 0, per_page: int = 25, timeout: int = 180
    ):
        super().__init__(timeout=timeout)
        self.bot = bot
        self.owner_id = owner_id
    ...
    def build_list_embed(self) -> discord.Embed:
        ...
        e.set_footer(
            text=f"Página {self.page_index + 1}/{self.total_pages()} • Selecciona un servidor en el menú para ver su ficha"
        )
        return e
...
class OwnerCog(commands.Cog):
    ...
    async def slashsync_cmd(self, ctx: commands.Context, scope: str = "global"):
        """Sincroniza slash commands o elimina copias antiguas por servidor."""
        ...
        if normalized is None:
            await ctx.reply(
                "Uso: `c!slashsync [global|guild|all|cleanup]`.",
        ...
        if normalized == "cleanup":
            ...
            summary.append(
                f"Copias slash antiguas eliminadas en {success}/{len(self.bot.guilds)} servidores. "
                "Los comandos globales se conservan."
            )
            if failed:
    ...
    @owner_only()
    async def presence_add(
        self, ctx: commands.Context, name: str, activity_type: str, status: str, *, text: str
    ):
